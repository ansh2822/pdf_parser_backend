...
from app.services.r2_storage import upload_markdown
    ...
    try:
        ...
        if len(pdf_bytes) > max_size:
            await websocket.send_text(
                f"FATAL: File too large ({len(pdf_bytes)} bytes). Maximum size is {max_size} bytes"
            )
            return
        ...
        if store:
            key = f"{filename}.md" if filename else f"{uuid.uuid4()}.md"
            try:
                url = upload_markdown(md_content, key)
                await websocket.send_text(f"R2_URL: {url}")
            except Exception as e:
                await websocket.send_text(f"R2_ERROR: {e}")
